...
The default Compose setup injects startup variables from the repository-root `.env` and points `ENV_FILE` at `/app/data/runtime.env` in the data volume. Runtime settings saved from the Web UI therefore persist in host file `data/runtime.env` across container recreation. Do not use a single-file `.env` bind mount. Same-name variables supplied through startup `env_file`, `--env-file`, `docker run -e`, or Compose `environment:` normally take precedence; update or remove those startup overrides when the Web UI value should take effect. `SCREENING_ENABLED` is the exception: once present in the runtime file, that persisted value wins so the Web screening toggle survives container recreation.
...
- **Recoverable stock and ETF screening** - The page supports regular A-shares and an independent mainland-equity-index ETF pool. When an exchange or cached code universe is available, ETF snapshots prefer batched Tencent quotes and fall back to Sina when coverage is incomplete; Sina still enumerates codes on a cold start. ETF trend, low-volatility, and oversold-recovery strategies use exchange eligibility, fine-grained exposure themes, isolated caches, and a Tencent/Sina/AkShare daily-data chain. ETF results keep one fund per theme and skip company fundamentals, stock news, and deep single-stock analysis. Background tasks and same-day results remain recoverable after navigation
