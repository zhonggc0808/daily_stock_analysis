...
- [修复] Docker 重建后优先恢复 Web 设置页持久化的选股开关，避免启动环境旧值导致左侧“选股”入口消失
- [新功能] Web 新增行情涨跌配色偏好、报告目录、选股分页对比与 CSV 导出、Chat 股票引用快捷操作
- [修复] 收紧前端异步轮询生命周期和选股取消竞态，后台标签页暂停任务轮询并在返回前台后同步
- [改进] 股票分析并行聚合基本面与本地情报并复用 Agent 上下文；选股全市场快照合并同参数并发请求，取消操作可中断候选补充、LLM 重试与后置分析，同时记录关键阶段耗时及缓存命中诊断
- [改进] 选股按有效交易日复用本地日线，并发补齐候选 DSA 行情、基本面、新闻和事件；API 个股分析不再现场生成大盘复盘，传统个股 LLM 使用覆盖主模型、备用模型及完整性重试的统一超时预算
- [改进] 优化股票分析性能：A 股日线优先使用 Tencent 并校验降级，按交易时段缓存实时行情，缓存新闻/筹码/基本面/板块数据，并发执行独立外部数据分支且保留串行回退开关
- [改进] 重新分析默认复用当日数据，并提供显式强制刷新入口
