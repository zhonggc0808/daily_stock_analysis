...
- 🧪 **选股任务可恢复** - 支持普通 A 股与独立 A 股 ETF 市场池；ETF 提供趋势、低波稳健和超跌修复策略，按境内指数主题去重并跳过公司基本面和个股深度分析。有交易所或缓存代码池时 ETF 快照优先使用腾讯批量行情，冷启动由新浪枚举代码，腾讯响应覆盖率不足时自动回退新浪。页面提交后台任务后可恢复进度或当日结果；普通 A 股热点与近分轮换行为保持不变
